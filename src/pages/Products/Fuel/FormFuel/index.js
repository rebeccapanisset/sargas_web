...
function FormFuel({ show, handleClose, selectedItem, update }) {
  const formRef = useRef();
  const { storeFuel, updateFuel } = useFuel();
  ...
  async function handleForm(data) {
    ...
    setLoading(true);
    data = { ...data, price: numAsString, datatype: 'fuel_tank' };
    if (await checkValidation(formRef, data, schema)) {
      const file = await storeFile(data.image, 'fuel');

      const formData = { ...data, image_id: file.id };

      const response = update
        ? await updateFuel(formData, selectedItem.product_id)
        : await storeFuel(formData);
