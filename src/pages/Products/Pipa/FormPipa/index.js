...
function FormPipa({ show, handleClose, selectedItem, update }) {
  const formRef = useRef();
  const { storePipa, updatePipa } = usePipa();
  ...
  async function handleForm(data) {
    ...
    setLoading(true);
    data = { ...data, price: numAsString, datatype: 'pipa_tank' };
    if (await checkValidation(formRef, data, schema)) {
      const file = await storeFile(data.image, 'pipa');

      const formData = { ...data, image_id: file.id };

      const response = update
        ? await updatePipa(formData, selectedItem.product_id)
        : await storePipa(formData);
...
FormPipa.defaultProps = {
  update: false,
};

export default FormPipa;
